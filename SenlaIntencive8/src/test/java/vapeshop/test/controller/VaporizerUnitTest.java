...
//        .header("Authorization", token)
    ...
    public void testAddVaporizer() throws Exception {
        char id=mockMvc.perform(post("/item/add").header("Authorization", token)
        ...
        MvcResult mvcResult = mockMvc.perform(post("/vaporizer/add").header("Authorization", token)
        ...
        MvcResult mvcResult1 = mockMvc.perform(get("/vaporizer/find/{id}", idVape).header("Authorization", token)).andReturn();
        Assertions.assertFalse(mvcResult1.getResponse().getContentAsString().isEmpty());
    }

    @Test
    public void testUpdateVape() throws Exception {
        mockMvc.perform(post("/vaporizer/update").header("Authorization", token)
                        .content(asJsonString(new VaporizerDTO(1,1.8,"испаритель",new ItemDTOInfoForCatalog(1))))
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isUpgradeRequired());
        MvcResult mvcResult1 = mockMvc.perform(get("/vaporizer/find/{id}", "1").header("Authorization", token)).andReturn();
    ...
    public void testGetAllVaporizer() throws Exception {
        MvcResult mvcResult = mockMvc.perform(get("/vaporizer/getAll").header("Authorization", token)).andReturn();
        System.out.println(mvcResult.getResponse().getContentAsString());
    }

    @Test()
    public void testDeleteVaporizer() throws Exception {
        mockMvc.perform(post("/item/add").header("Authorization", token)
                        .content(asJsonString(new ItemDTOFullInfo("photo4", "HotSpot BubleGum", new Category("Испарители,Картриджы,Койлы"), new BigDecimal(Double.toString(23.0)), 15)))
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
        ...
        MvcResult mvcResult = mockMvc.perform(post("/vaporizer/add").header("Authorization", token)
                ...
                .andExpect(status().isCreated()).andReturn();
        char idVape = mvcResult.getResponse().getContentAsString().charAt(6);
        MvcResult mvcResult1 = mockMvc.perform(post("/vaporizer/delete/{id}", idVape).header("Authorization", token)).andReturn();
