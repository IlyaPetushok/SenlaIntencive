...
import project.vapeshop.dao.impl.OrderDao;
...
public class RatingUnitTest {
    ...
    public void testGetAllOrders(){
        ...
        when(ratingDao.selectObjects()).thenReturn(list);
        List<RatingDTOForProduct> ratings=ratingService.showObjects();
        for (int i = 0; i < ratings.size(); i++) {
            Assertions.assertEquals(ratings.get(i).getId(),list.get(i).getId());
        }
        verify(ratingDao,times(1)).selectObjects();
    }

    @Test
    public void testUpdateOrder(){
        when(ratingDao.update(any(Rating.class))).thenReturn(rating);
        Assertions.assertEquals(ratingService.updateObject(ratingDTOFullInfo).getId(),rating.getId());
